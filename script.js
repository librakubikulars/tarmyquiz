...
startBtn.onclick = () => {
  ...
  if(!name){ alert("Lütfen isminizi gir aşkım 💅🏻"); return; }
...
function renderQuestions(){
  ...
  questionCounter.innerText = `Soru 1 / ${questions.length}`;
}
...
window.selectOption = (qIndex,optIndex,btn)=>{
  ...
  questionCounter.innerText = `Soru ${Object.keys(userAnswers).length} / ${questions.length}`;
}
...
finishBtn.onclick = ()=>{
  ...
  certTitle.innerText = `${name}’in Twitter T-ARMY Cehennemi Sertifikası`;
